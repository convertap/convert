...
G7 reports twelve subchecks separately rather than one verdict, because they became real at different moments and a summary line would let the others read as proven (ADR 0048). Each is tagged **real**, **waiting** or **conditional**, and the script prints all three counts. **All twelve are real since the first migration** on 22 August 2026: nothing waits, and the two that were conditional now fire. `table ownership` fires because the right question was never whether `convert_app` owns a policed table, which `bootstrap.sql` forbids, but who *does* own each one and whether the application can become or inherit them. `definer routines` fires because the identity read path of ADR 0054 gave it routines to check.

The twelfth subcheck, `schema boundary`, exists because every other query in the file reads `public` alone and nothing said so. Review created a plain owner-context view in another schema over `public."user"` and read every account while the gate reported zero views. Outside the system schemas only `public`, which is inventoried, and `drizzle`, which the application cannot reach, may exist. **What G7 still does not assert is listed in ADR 0054's Enforcement section**, because a gate's gaps belong in writing rather than in the next person's afternoon. `TABLE_ACCESS` (ADR 0050) replaced `TENANT_TABLES` and `NON_TENANT_TABLES`, which classified by whether a table carried a `workspace_id` column and so had nothing to say about a table protected some other way.
...
- **Requests are validated at the boundary.** A strict global Zod pipe rejects controller parameters that are not backed by a derived DTO, and `ErrorFilter` maps Zod issues to dotted `details: FieldError[]` paths. Response serialization uses the same schemas, so an invalid handler result is an internal error rather than an undocumented wire shape (ADR 0045).

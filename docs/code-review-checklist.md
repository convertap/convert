...
- [ ] **Tenancy.** Every new table has `org_id NOT NULL` and an RLS policy. No query bypasses RLS. No test disables RLS to pass — it sets the org context instead.
...
- [ ] The send path checks consent for marketing categories — in the path, not in the UI, which the API and worker bypass (I9).

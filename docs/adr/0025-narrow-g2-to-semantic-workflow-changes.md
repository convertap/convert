...
Verified against the real pull requests rather than a synthetic case — the four Dependabot workflow bumps (#2–#5) pass under the new comparison, and PR #24, which widened the push trigger to `develop`, still requires its ADR and still fails without one.

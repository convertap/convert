...
**Re-proven on PostgreSQL 18 (ADR 0053).** This record's probe named no server version, so it silently carried the Postgres 16 it was run on — the same overstatement ADR 0048 is about, one level down. Re-run on 18.6 the probe produces the same seven named failures and exit 1, and the revoked-`UPDATE` inverse still fails. ADR 0053 holds the current evidence.

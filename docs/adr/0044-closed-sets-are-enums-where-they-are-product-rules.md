...
**Re-proven on PostgreSQL 18 (ADR 0053).** Enum ordering was verified here against Postgres 16, two majors behind the deployed version. On 18.6 `order by stage` still gives declaration order and `order by stage::text` still gives alphabetical, so the reason the enum won is intact. ADR 0053 holds the current evidence.

...
**Re-proven on PostgreSQL 18 (ADR 0053).** The version this record measured, Postgres 16, is two majors behind what serves traffic; CI and local development moved to 18 on 22 August 2026. The policy expression the gate derives is byte-identical on 18.6, the empty-schema run reports the same three-of-ten, and the naive form still raises on an empty context instead of returning nothing. This record's Decision stands unchanged; ADR 0053 holds the current evidence.

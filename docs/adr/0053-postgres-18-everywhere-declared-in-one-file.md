...
was confirmed on 21 August 2026 by reading the Railway services, where `staging` and `test` each run
...
2. no line in those six files names a *different* major unless that line carries
   `pg-version:historical=<major>`, naming the major it excuses. That marks a deliberate reference to the
...
18's official image made `PGDATA` version-specific, at `/var/lib/postgresql/18/docker`, and moved its
...
- *Pin Railway to 16 and leave CI alone.* What runs would then be what is proven, and today it is
  ...
  a shorter supported life, chosen for no reason except matching a CI file. Railway majors are also
...
The hatch **names the majors it excuses**, as `pg-version:historical=16`, rather than excusing a
...
case, where `UPDATE` is revoked while `updated_at` is present, still fails with

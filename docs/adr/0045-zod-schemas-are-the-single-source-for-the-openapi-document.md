...
**What exists and what does not, as of this record.** Built: the `.boundaries.json` rule that forbids `nestjs-zod` and `@nestjs/swagger` in `contracts`, and `tools/check_openapi_complete.py`, which is G10's completeness half and was verified by making it fail. **Not built: everything else in the Decision above.** `nestjs-zod` is not a dependency of `apps/api`. There are no `*.dto.ts` files, no `ZodValidationPipe`, no `cleanupOpenApiDoc`, no `@ZodResponse`, no `details` branch in `ErrorFilter`, and `pagination.ts` still calls `Buffer` at line 22 — the browser-imported Node global this record says is removed. Concretely, as of this branch: `apps/api/src/openapi/document.ts` calls `SwaggerModule.createDocument` with no `cleanupOpenApiDoc` wrapper, `apps/api/src/interface/http/health.controller.ts` declares `@ApiOkResponse`, `apps/api/src/app.module.ts` registers no global pipe, and `apps/api/package.json` lists no `nestjs-zod`. The file pointers are here so the next reader can check the claim in a minute rather than trusting it. This paragraph exists because the first draft of this record read as though the wiring had shipped, and a decision record that overstates its own enforcement is the most expensive kind of wrong document this repository can hold — it retires a question that is still open. The decision stands; the commit that implements it is a separate piece of work.
...
- **G10 now also requires a 4xx or 5xx response using `ErrorEnvelope` on every operation.** This closes the omission named in this record once the Zod-derived envelope became available to controllers.
- **The API boundary now uses the accepted adapter:** `nestjs-zod` is pinned in `apps/api`; global strict request validation and response serialization are registered; DTO classes derive from shared schemas; `@ZodResponse` and `cleanupOpenApiDoc` keep runtime, type and OpenAPI shapes aligned; validation failures populate dotted field details.
- **`packages/contracts/src/pagination.ts` no longer uses `Buffer`.** A cursor is a validated ULID string, and focused tests hold malformed-cursor rejection and page-size bounds.

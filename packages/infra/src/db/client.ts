...
/**
 * Database access, and the one place the organisation context is set.
 *
 ...
 * as the statements that follow it. `withOrgContext` is therefore the only way to obtain a
 * transaction - acquiring a connection and setting its context cannot be separated,
 * because doing so is how one organisation's data leaks into another's request.
 */
 ...
 */
export const withoutOrgContext = async <T>(db: Database, fn: (db: Database) => Promise<T>) =>
  fn(db);

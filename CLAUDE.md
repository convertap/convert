...
5. **`docs/adr/`**. 52 decision records. **Accepted:** 0001, 0015–0052. **Proposed:** 0002, 0003, 0005–0014. **Superseded:** 0004, by 0043; the branch-model portions of 0019 and 0024, by 0049; the table-classification inventory of 0042 and both `IDENTITY_TABLES` and the `verification_attempt` access mechanism of 0047, by 0050; 0050's refusal of views, by 0051. Cite by number. Never edit an accepted **Decision**, supersede it — the header and the Enforcement section may be edited, and ADR 0048 requires the Enforcement section to say what exists today rather than what is intended.
...
|------|-------------------------|-----------|
| ~~G7 migrations and RLS~~ | **Partly vacuous, and it now says which parts, per subcheck.** Nine subchecks, one line each, tagged real or vacuous, with the count printed (ADR 0050). **Three real:** the application role is neither superuser nor BYPASSRLS and no default privilege grants rights on future tables; every table declared in `schema.ts` is classified in `TABLE_ACCESS` — one today, `workspace`; and a cross-tenant read on a fixture table returns nothing while the owner sees both rows (ADR 0042). **Six vacuous:** table ownership, registry-to-catalogue, the tenancy graph, both policy classes, and effective privileges — all need a public table | each vacuous subcheck becomes real with the first migration that gives it a table |
